...
#define GPIO_INTERRUPT_PIN_A    6    /* GPIO pin number mapped to interrupt */
#define GPIO_INTERRUPT_PORT_A   GPIOINT_PORT2	/* GPIO port number mapped to interrupt */

#define GPIO_INTERRUPT_PIN_B    7    /* GPIO pin number mapped to interrupt */
#define GPIO_INTERRUPT_PORT_B   GPIOINT_PORT2	/* GPIO port number mapped to interrupt */
...
bool PUSH_BUTTON(int push)
{
bool state = false;

if (push == 0)
{
state = Chip_GPIO_ReadPortBit(LPC_GPIO, GPIO_INTERRUPT_PORT_A, GPIO_INTERRUPT_PIN_A);
}
else if (push == 1)
{
state = Chip_GPIO_ReadPortBit(LPC_GPIO, GPIO_INTERRUPT_PORT_B, GPIO_INTERRUPT_PIN_B);
}
return state;
}
...
{
	Chip_GPIOINT_ClearIntStatus(LPC_GPIOINT, GPIO_INTERRUPT_PORT_A, 1 << GPIO_INTERRUPT_PIN_A);
	bool A= PUSH_BUTTON(0);
	bool B= PUSH_BUTTON(1);
...
int main(void)
{
	...
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, GPIO_INTERRUPT_PORT_B, GPIO_INTERRUPT_PIN_B);


	/* Configure the GPIO interrupt */
	Chip_GPIOINT_SetIntFalling(LPC_GPIOINT, GPIO_INTERRUPT_PORT_A,1 << GPIO_INTERRUPT_PIN_A);
